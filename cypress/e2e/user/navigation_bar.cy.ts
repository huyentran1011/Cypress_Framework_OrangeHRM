...
describe("Navigation Bar under User account", () => {

    beforeEach(() => {
        cy.fixture('navigationBar/menuItemsForUser').then(data => menuItemsData = data.menuItems);
        cy.loginAsUser();
    })
    ...
    it(`Verify Navigation Menu actions under User account`, () => {
        ...
        const tempMenuItems = menuItemsData.filter((item: string) => item !== 'Leave');

        tempMenuItems.forEach((menuItem: string) => {
...
describe("Search function in User Navigation Bar", () => {
    beforeEach(() => {
        cy.fixture('navigationBar/menuItemsForUser').then(data => menuItemsData = data.menuItems);
...
describe('Toggle Button in User Navigation Bar', () => {
    beforeEach(() => {
        cy.loginAsUser();
        cy.fixture('navigationBar/menuItemsForUser').then(data => menuItemsData = data.menuItems);
    })
